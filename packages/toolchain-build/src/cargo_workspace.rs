...
pub fn create_isolated_cargo_manifest(
    ...
    create_isolated_cargo_manifest_with_deps_removed(
        original_manifest_path, members, std::iter::empty::<String>()
    )
}

pub fn create_isolated_cargo_manifest_with_deps_removed<I: IntoIterator<Item=impl Into<String>>>(
    original_manifest_path: &Path,
    members: Option<&str>,
    dep_to_remove: I
) -> cu::Result<String> {
    let dep_to_remove = dep_to_remove.into_iter()
        .map(|x|x.into()).collect::<BTreeSet<_>>();
    ...
    cu::check!(cu::fs::read_string(&workspace_manifest_path), "failed to read workspace Cargo.toml")?;
    let workspace_manifest = toml::parse::<toml::Table>(&workspace_manifest)?;

    // load and patch [workspace] in for the crate manifest
    let mut manifest = 
    cu::check!(cu::fs::read_string(original_manifest_path), "failed to read original crate Cargo.toml")?;
    ...
    for dep_name in inherited_deps {
        ...
        if let Some(data) = workspace_dep_data.as_table() {
            if data.get("path").is_some() {
                cu::bail!("workspace dep cannot be a path when creating isolated crate: {dep_name}");
            }
        }
        workspace_deps_to_add.insert(dep_name, workspace_dep_data.clone());
    }
    let workspace = cu::check!(manifest["workspace"].as_table_mut(), "unexpected: [workspace] not found in generated manifest")?;
    workspace.insert("dependencies".to_string(), workspace_deps_to_add.into());

    cu::check!(toml::stringify_pretty(&manifest), "failed to serialize generated manifest")
}
fn remove_dependencies(
    ...
    let deps = cu::check!(deps.as_table_mut(), "dependencies map must be a table (for key '{dep_key}')")?;
    for r in remove {
        let _ = deps.remove(r);
    }
    Ok(())
...
fn collect_inherited_dependencies(
    ...
    let deps = cu::check!(deps.as_table(), "dependencies map must be a table (for key '{dep_key}')")?;
