...
fn basic_tests(mtt: &mut MegatonTests) {
    ...
    if path.exists() {
        ...
        if mtt
            .megaton_assert_ok(result, "Test \"basic_tests\": Failed to remove file!\n")
            .is_none()
        {
            return;
        }
    ...
    mtt.megaton_assert_msg(
        result.is_some(),
        true,
        "Test \"basic_tests\": File::create returned None, expected Some",
    );
    if result.is_none() {
        return;
    }
...
fn test_consecutive_writes(mtt: &mut MegatonTests) {
    ...
    let open_result = OpenOptions::new()
        ...
        .write(true)
        .open(&path);
    let result = mtt.megaton_assert_ok(
        open_result,
        "Test \"test_consecutive_writes\": Failed to create file!\n",
    );
    ...
    mtt.megaton_assert_ok(
        result,
        "Test \"test_consecutive_writes\": Failed to write to file\n",
    );

    let result = test_file.write(LINES[1]);
    mtt.megaton_assert_ok(
        result,
        "Test \"test_consecutive_writes\": Failed to write second chunk to file\n",
    );

    let read_back = std::fs::read(&path);
    if let Some(content) = mtt.megaton_assert_ok(
        read_back,
        "Test \"test_consecutive_writes\": Failed to read back file after consecutive writes\n",
    ) {
        mtt.megaton_assert_msg(
            content.as_slice(),
            TOTAL_CONTENT,
            "Test \"test_consecutive_writes\": Consecutive writes did not append correctly",
        );
    }
}
...
fn test_write_seek_offset(mtt: &mut MegatonTests) {
    ...
    if let Some(mut file) = mtt.megaton_assert_ok(
        result,
        "Test \"test_write_seek_offset\": Failed to recreate file for seek offset test\n",
    ) {
        let result = file.write(LINES[0]);
        if mtt
            .megaton_assert_ok(
                result,
                "Test \"test_write_seek_offset\": Failed to write for seek offset test\n",
            )
            .is_some()
        {
            ...
            if let Some(bytes) = mtt.megaton_assert_ok(
                content,
                "Test \"test_write_seek_offset\": Failed to read back file\n",
            ) {
                mtt.megaton_assert_msg(
                    bytes.len(),
                    LINES[0].len(),
                    "Test \"test_write_seek_offset\": File length wrong after write",
                );
            }
        }
    }
...
fn test_close_frees_fd(mtt: &mut MegatonTests) {
    let path: PathBuf = PathBuf::from("sd:/testfile.txt");
    let result = File::create(&path);
    if let Some(file) = mtt.megaton_assert_ok(
        result,
        "Test \"test_close_frees_fd\": Failed to create file for close test\n",
    ) {
        drop(file);
        let result = File::open(&path);
        mtt.megaton_assert_ok(
            result,
            "Test \"test_close_frees_fd\":  Failed to reopen file after close - fd was not freed\n",
        );
    }
...
fn test_multiple_files(mtt: &mut MegatonTests) {
    ...
    let file1 = mtt.megaton_assert_ok(
        File::create(&path),
        "Test \"test_multiple_files\": Failed to open file 1\n",
    );
    let file2 = mtt.megaton_assert_ok(
        File::create(&path2),
        "Test \"test_multiple_files\": Failed to open file 2\n",
    );
    let file3 = mtt.megaton_assert_ok(
        File::create(&path3),
        "Test \"test_multiple_files\": Failed to open file 3\n",
    );
...
fn test_read_seek_offset(mtt: &mut MegatonTests) {
    ...
    if mtt
        .megaton_assert_ok(file.write(TOTAL_CONTENT), "Failed to write to file\n")
        .is_none()
    {
        return;
    }
    ...
    if let Some(mut file) =
        mtt.megaton_assert_ok(result, "Failed to open file for read seek test\n")
    {
        let mut buf1 = vec![0u8; LINES[0].len()];
        let result = file.read(&mut buf1);
        if mtt
            .megaton_assert_ok(result, "Failed to read first chunk\n")
            .is_some()
        {
            ...
            if mtt
                .megaton_assert_ok(result, "Failed to read second chunk\n")
                .is_some()
            {
