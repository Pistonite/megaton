...
inline_always_ void install_replace_hook_at_offset(ptrdiff_t target,
                                                   uintptr_t callback) {
    do_install_hook_at_offset(target, callback, false);
}
inline_always_ uintptr_t install_trampoline_hook(uintptr_t target,
                                                 uintptr_t callback) {
    return do_install_hook(target, callback, true);
}
inline_always_ uintptr_t install_trampoline_hook_at_offset(ptrdiff_t target,
                                                           uintptr_t callback) {
...
#define assert_hook_callback_defined_                                          \
    static_assert(                                                             \
        !std::is_member_function_pointer_v<decltype(&TDerived::call)>,         \
        "missing definition for `call` or missing `static` specifier for "     \
        "hook callback");
#define assert_derived_hook_callback_defined_                                  \
    static_assert(!std::is_member_function_pointer_v<decltype(&call)>,         \
                  "missing definition for `call` or missing `static` "         \
                  "specifier for hook callback");

/** Inline hook trait */
template <typename TDerived> struct inline_hook {
    ...
    inline_member_ static void install_at_offset(ptrdiff_t main_offset) {
        assert_hook_callback_defined_;
        install_inline_hook_at_offset(
            main_offset, reinterpret_cast<uintptr_t>(TDerived::call));
    }
};

/** Replace hook trait */
template <typename TDerived> struct replace_hook {
    inline_member_ static void install_at(uintptr_t target) {
        assert_hook_callback_defined_;
        install_replace_hook(target,
                             reinterpret_cast<uintptr_t>(TDerived::call));
    }
    inline_member_ static void install_at_offset(ptrdiff_t main_offset) {
        assert_hook_callback_defined_;
        install_replace_hook_at_offset(
            main_offset, reinterpret_cast<uintptr_t>(TDerived::call));
    }
};

/** Trampoline hook trait */
template <typename TDerived> struct trampoline_hook {
    ...
    inline_member_ static void install_at(uintptr_t target) {
        assert_hook_callback_defined_;
        trampoline() =
            reinterpret_cast<decltype(&TDerived::call)>(install_trampoline_hook(
                target, reinterpret_cast<uintptr_t>(TDerived::call)));
    }
    inline_member_ static void install_at_offset(ptrdiff_t main_offset) {
        assert_hook_callback_defined_;
        trampoline() = reinterpret_cast<decltype(&TDerived::call)>(
            install_trampoline_hook_at_offset(
                main_offset, reinterpret_cast<uintptr_t>(TDerived::call)));
    }
