...
mod ffi {
    pub fn log_success(i: i32) {
        print!("Success! {i}");
    }
}


fn success_result(i: i32){
    ffi::log_success(i);
}
fn error_result(i: i32){
    print!("Success! {i}");
}
fn inc(mut i: i32) -> i32 {
    let temp = i;
    i += 1;
    temp
}
...
mod megaton {
    pub(crate) fn debug_show_fd_list() {
        println!("foo!");
    }
}
...
mod tests {
    ...
    #[test]
    fn test_new(){
        let path: PathBuf = PathBuf::from("testfile3.txt");
        ...
        const lines: [&[u8]; 2] = ["Hello world!\n".as_bytes(), "This is a test!".as_bytes()];
        const total_len: usize = total_content.len();
        {
            // Test exists/delete
            if path.exists() {
                let res = std::fs::remove_file(&path);
                if res.is_err() { error_result(test_id);  }
                else { success_result(test_id); }
            } else {
                success_result(test_id);
            }
            test_id += 1;
            ...
            for i in 0..2 {
                print!("{:?}\n", lines[i]);
                let res = f.write(lines[i]);
                if res.is_err() {
                    return error_result(inc(test_id));
                } else {
                    success_result(inc(test_id));
                }
            }
        ...
        
        {
            ...
            if res.is_err() {
                f.write(b"\nUnexpected value returned from read: \n").unwrap();
                f.write(&buf).unwrap();
                f.flush().unwrap();
                return error_result(inc(test_id))
            } else {
        ...
        { 
            ...
            let path2 = PathBuf::from("testfile4.txt");
            let res = File::open(&path);
            let res = OpenOptions::new().append(true).open(&path);
            let res2 = OpenOptions::new().create(true).write(true).open(&path2);
            if res.is_err() { return error_result(inc(test_id)); } else { success_result(inc(test_id)); }
            if res2.is_err() { return error_result(inc(test_id)); } else { success_result(inc(test_id)); }
            
            let mut f1 = res.unwrap();
            let mut f2 = res2.unwrap();
            megaton::debugShowFDList();
            let content1 = "Appended content".as_bytes();
            let content2= "Multiple files open at once!".as_bytes();
            let res = f1.write(content1);
            let res2 = f2.write(content2);
            if res.is_err() { return error_result(inc(test_id)); } else { success_result(inc(test_id)); }
            if res2.is_err() { return error_result(inc(test_id)); } else { success_result(inc(test_id)); }
        }
    }
