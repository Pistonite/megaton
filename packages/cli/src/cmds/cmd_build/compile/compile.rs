...
use crate::{cmds::cmd_build::{compile::{source_file::{discover_source_files}}, compile_db::{CompileDB}, config::Flags}};
...
pub async fn run_compilation(
    ...
    let sources = sources.iter().flat_map(|source_directory| {
        let discovered_source_files = discover_source_files(source_directory);
        let _ = discovered_source_files.as_ref().inspect_err(|e| cu::error!("Failed to discover sources in {:?}: {}", source_directory, e));
