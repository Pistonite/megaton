...
extern "C" void __megaton_lib_log(const uint8_t* buf, uint64_t len) {
    ...
    if (log_file != nullptr) {
        fwrite(buf, sizeof(uint8_t), len, log_file);
        fclose(log_file);
    }
}
...
extern "C" void __megaton_lib_fs_write_stdout(const uint8_t* buf, uint64_t len) {
    ...
    if (stdout_file != nullptr) {
        fwrite(buf, sizeof(uint8_t), len, stdout_file);
        fclose(stdout_file);
    }
}
...
extern "C" void __megaton_lib_fs_write_stderr(const uint8_t* buf, uint64_t len) {
    ...
    if (stderr_file != nullptr) {
        fwrite(buf, sizeof(uint8_t), len, stderr_file);
        fclose(stderr_file);
    }
}
...
extern "C" NNResult __megaton_lib_fs_write_file(uint64_t nn_fd, const uint8_t* buf, uint64_t size,
                                                size_t position) {
    nn::fs::WriteOption option = nn::fs::WriteOption::CreateOption(nn::fs::WriteOptionFlag_Flush);
    nn::Result result = nn::fs::WriteFile({nn_fd}, s64(position), (void*)buf, size, option);
    if (!result.IsSuccess()) {
...
int32_t hermit_to_nn_flags(uint32_t hermit_open_option_flags) {
    // hermit OpenOption defintion:
    // https://github.com/hermit-os/kernel/blob/ec0fc3572c9d8deba725df8b7eb000980034a9f6/src/fd/mod.rs#L53
    // nnheaders OpenOption definition:
    // https://github.com/open-ead/nnheaders/blob/0547381a6166ea54fb306a53a02683a8527475fd/include/nn/fs/fs_types.h#L51

    int32_t nn_flags = 0;
    auto access = hermit_open_option_flags & 0b11;
    if (access == 0) {
        nn_flags |= nn::fs::OpenMode_Read;
    } else if (access == 1) {
        nn_flags |= nn::fs::OpenMode_Write;
    } else if (access == 2) {
        nn_flags |= nn::fs::OpenMode_ReadWrite;
    }
    if (access != 0) {
        nn_flags |= nn::fs::OpenMode_Append;
    }
    return nn_flags;
}
...
extern "C" OpenResult __megaton_lib_fs_open(const char* name, int32_t flags, uint32_t mode) {
    ...
    open_result.fd = {.inner = 0, .kind = FileDescriptorType::FILE, .seek_offset = 0};
    ...
    if (opened_type == nn::fs::DirectoryEntryType_File) {
        auto open_mode = hermit_to_nn_flags(flags);
...
extern "C" ReadResult __megaton_lib_fs_read_file(uint64_t nn_fd, uint64_t seek_pos, char* buf,
                                                 uint64_t len) {
    uint64_t bytes_read;
...
extern "C" GetEntryTypeResult __megaton_lib_fs_get_entry_type(const char* name) {
    ...
    GetEntryTypeResult entry_type_result = {.result = build_simple_result(result),
                                            .entry_type = nn::fs::DirectoryEntryType_File};
    if (result.IsSuccess()) {
        entry_type_result.entry_type = type;
    }
