...
static Result _memregionInitWithInfo(MemRegion* r, InfoType id0_addr,
                                     InfoType id0_sz) {
    u64 base;
...
static void _memregionInitHardcoded(MemRegion* r, uintptr_t start,
                                    uintptr_t end) {
    r->start = start;
    r->end = end;
}
...
#ifndef MEGATON_LIB
NX_INLINE bool _memregionIsInside(MemRegion* r, uintptr_t start,
                                  uintptr_t end) {
    return start >= r->start && end <= r->end;
}
#endif

NX_INLINE bool _memregionOverlaps(MemRegion* r, uintptr_t start,
                                  uintptr_t end) {
    return start < r->end && r->start < end;
}

NX_INLINE bool _memregionIsMapped(uintptr_t start, uintptr_t end,
                                  uintptr_t guard, uintptr_t* out_end) {
...
NX_INLINE bool _memregionIsReserved(uintptr_t start, uintptr_t end,
                                    uintptr_t guard, uintptr_t* out_end) {
    // Adjust start/end by the desired guard size.
    start -= guard;
    end += guard;
...
static void* _memregionFindRandom(MemRegion* r, size_t size,
                                  size_t guard_size) {
    ...
    for (unsigned i = 0; i < RANDOM_MAX_ATTEMPTS; i++) {
        ...
        for (;;) {
            uintptr_t page_offset =
                __libnx_virtmem_rng() % (aslr_max_page_offset + 1);
...
void virtmemSetup(void) {
    ...
    rc = _memregionInitWithInfo(&g_AliasRegion, InfoType_AliasRegionAddress,
                                InfoType_AliasRegionSize);
    if (R_FAILED(rc)) {
        // Wat.
// megaton: use own panic
#ifdef MEGATON_LIB
        panic_nx_("init alias region failed",
    ...
    rc = svcGetInfo(&alias_extra_size, InfoType_AliasRegionExtraSize,
    ...
    rc = _memregionInitWithInfo(&g_HeapRegion, InfoType_HeapRegionAddress,
                                InfoType_HeapRegionSize);
    if (R_FAILED(rc)) {
        // Wat.
// megaton: use own panic
#ifdef MEGATON_LIB
        panic_nx_("init heap region failed",
    ...
    // available [2.0.0+]
    rc = _memregionInitWithInfo(&g_AslrRegion, InfoType_AslrRegionAddress,
                                InfoType_AslrRegionSize);
    if (R_SUCCEEDED(rc)) {
        rc = _memregionInitWithInfo(&g_StackRegion, InfoType_StackRegionAddress,
...
// megaton: use own panic
        ...
        rc = svcUnmapMemory((void*)0xFFFFFFFFFFFFE000UL, (void*)0xFFFFFE000UL,
                            0x1000);
        if (R_VALUE(rc) == KERNELRESULT(InvalidMemoryState)) {
            ...
            // Thus we are 36-bit.
            _memregionInitHardcoded(&g_AslrRegion, 0x8000000ull,
                                    0x1000000000ull);
            _memregionInitHardcoded(&g_StackRegion, 0x8000000ull,
                                    0x80000000ull);
        } else {
            // Wat.
// megaton: use own panic
...
// megaton: no mutex
    ...
    return _memregionFindRandom(
        g_IsLegacyKernel ? &g_StackRegion : &g_AslrRegion, size, guard_size);
}
...
// megaton: no mutex; use program's malloc
    ...
    VirtmemReservation* rv =
        (VirtmemReservation*)malloc(sizeof(VirtmemReservation));
#else
    if (!mutexIsLockedByCurrentThread(&g_VirtmemMutex))
        return NULL;
    VirtmemReservation* rv =
        (VirtmemReservation*)__libnx_alloc(sizeof(VirtmemReservation));
#endif
    if (rv) {
