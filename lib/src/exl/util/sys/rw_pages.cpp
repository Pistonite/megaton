...
static void ForEachMemRange(Callback callback, uintptr_t start_address,
                            size_t length) {
    const uintptr_t end = start_address + length;
    /* Setup variables. */
    MemoryInfo meminfo{.addr = start_address};
    u32 pageinfo;
    ...
    do {
        ...
        uintptr_t offset =
            std::max(meminfo.addr, start_address) - start_address;
        ...
        uintptr_t current_length =
            std::min(end, meminfo.addr + meminfo.size) - current_address;
...
RwPages::RwPages(uintptr_t ro, size_t size) {
    ...
    /* Reserve rw region. */
    auto reserve =
        virtmemAddReservation((void*)alignedRw, claim.GetAlignedSize());
    ...
    ForEachMemRange(
        [alignedRw, procHandle](uintptr_t address, uintptr_t size,
                                uintptr_t offset) {
...
RwPages::~RwPages() {
    ...
    ForEachMemRange(
        [&claim, procHandle](uintptr_t address, uintptr_t size,
                             uintptr_t offset) {
