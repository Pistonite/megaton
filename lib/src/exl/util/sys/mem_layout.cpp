...
static void FindModules() {
    ...
    do {
        ...
        case State::ExpectingData: {
            ...
            /* This only needs to be determined at runtime if we are in an
             * application. */
...
static void InferAslrAndStack() {
    Result rc = svcUnmapMemory((void*)0xFFFFFFFFFFFFE000UL,
                               (void*)0xFFFFFE000UL, 0x1000);
...
static void FindRegions() {
    util::mem_layout::s_Alias =
        util::Range(GetAddressFromInfo(InfoType_AliasRegionAddress),
                    GetAddressFromInfo(InfoType_AliasRegionSize));

    util::mem_layout::s_Heap =
        util::Range(GetAddressFromInfo(InfoType_HeapRegionAddress),
                    GetAddressFromInfo(InfoType_HeapRegionSize));

    if (R_FAILED(TryGetAddressFromInfo(InfoType_AslrRegionAddress,
                                       &util::mem_layout::s_Aslr.m_Start)) ||
        R_FAILED(TryGetAddressFromInfo(InfoType_AslrRegionSize,
                                       &util::mem_layout::s_Aslr.m_Size)) ||
        R_FAILED(TryGetAddressFromInfo(InfoType_StackRegionAddress,
                                       &util::mem_layout::s_Stack.m_Start)) ||
        R_FAILED(TryGetAddressFromInfo(InfoType_StackRegionSize,
                                       &util::mem_layout::s_Stack.m_Size))) {
        InferAslrAndStack();
    }
}
