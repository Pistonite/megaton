...
inline_always_ std::pair<const InlineHookEntry*, InlineHookEntry*> allocate_inline_hook_entry() {
    ...
    auto ro_array = reinterpret_cast<const InlineHookEntry*>(s_inline_jit_pool.ro_start());
...
void install_inline_hook(uintptr_t hook, uintptr_t callback) {
    ...
    uintptr_t trampoline_code = reinterpret_cast<uintptr_t>(&entry_rx->trampoline);
    ...
    entry_rw->trampoline = {
        ...
        exl::armv8::inst::BranchLink(inline_hook_entrypoint() - (trampoline_code + (1 * sizeof(exl::armv8::InstType)))),
        /* Restore proper LR. */
        exl::armv8::inst::LdurUnscaledImmediate(
            exl::armv8::reg::LR, exl::armv8::reg::SP, lrBackupOffset),
        /* Branch to trampoline. */
        exl::armv8::inst::Branch(trampoline_ptr - (trampoline_code + (3 * sizeof(exl::armv8::InstType))))};
