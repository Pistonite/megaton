...
template <typename R, typename... A> using GenericFuncPtr = R (*)(A...);

namespace member_func {

template <typename T, typename R, typename... A> using Ptr = R (T::*)(A...);

template <typename T, typename R, typename... A>
using ConstPtr = R (T::*)(A...) const;

/* Itanium ABI. */
template <typename T, typename R, typename... A> struct RichImpl {
    using Generic = GenericFuncPtr<R, T*, A...>;

    uintptr_t m_Ptr;
    ptrdiff_t m_Adj;

    constexpr bool IsVirtual() const { return (m_Ptr & 1) == 1; }
...
template <typename T> struct Rich;
...
template <typename T> static constexpr auto Adapt(T in) {
