...
#define JIT_CREATE(name, size)                                                 \
    namespace impl::name {                                                     \
    __attribute__((section(".text." #name))) alignas(                          \
        PAGE_SIZE) static const std::array<u8, size> s_Area{};                 \
    }                                                                          \
...
public:
    ...
    inline void Initialize() {
        util::ConstructAt(m_Pages, reinterpret_cast<uintptr_t>(m_Rx.data()),
                          m_Rx.size());
    }

    inline void Flush() { GetPages().Flush(); }
