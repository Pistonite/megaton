...
noreturn_ __megaton_handle_panic_nx_result(const char* file, u32 line, const char* msg, u32 result);
...
#define assert_(expr) \
    do { \
        if (!static_cast<bool>(expr)) { \
            __megaton_handle_panic(__FILE__, __LINE__, "assertion failed: " #expr); \
        } \
    } while (0)
